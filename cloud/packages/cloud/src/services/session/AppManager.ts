    ...
    if (!session) {
      session = new AppSession({
        ...
        onDisconnect: (code: number, reason: string) => {
          ...
          if (this.disposed) {
            this.logger.debug({packageName, code, reason}, `[AppManager] Ignoring onDisconnect callback after disposal`)
            return
          }
    ...
    try {
      ...
      for (const [packageName, appSession] of this.apps) {
        try {
          ...
          this.logger.error({error, packageName}, `[AppManager:dispose] Error disposing AppSession for ${packageName}`)
        }
      }
      this.apps.clear()
    } catch (error) {
