...
```typescript
import { AppServer } from '@mentra/mentraos-sdk';
...
app.onSession(async (session, sessionId, userId) => {
  // Display "Hello World" on the glasses
  session.layouts.showTextWall({
    text: "Hello World from MentraOS!"
  });
...
3. Prints everything the user says to your server console.
...
🚀 Ready to build your first app? Checkout the [Quickstart](/quickstart).
