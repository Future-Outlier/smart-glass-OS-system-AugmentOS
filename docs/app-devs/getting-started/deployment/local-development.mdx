...
```
User's Glasses ←→ MentraOS Cloud ←→ Your App Server
```

**The flow:**
1. User interacts with glasses (voice, buttons, camera)
2. MentraOS Cloud processes the interaction
3. Cloud sends events to **your app server** via webhook
4. Your app responds (display text, play audio, etc.)
5. Response flows back through Cloud to glasses

**The key requirement:** MentraOS Cloud must be able to reach your app server over the internet.
...
Your App: http://localhost:3000 ✅ (you can access it)
         http://localhost:3000 ❌ (MentraOS Cloud cannot access it)
```

**Why this doesn't work:**
- `localhost` is not a public address
- MentraOS Cloud runs on the internet
- Cloud can't reach your local machine directly
...
```
MentraOS Cloud → https://abc123.ngrok.io → localhost:3000 → Your App
                 (public internet)         (your machine)
```

Now MentraOS Cloud can send events to `https://abc123.ngrok.io`, and they'll be forwarded to your local app.
