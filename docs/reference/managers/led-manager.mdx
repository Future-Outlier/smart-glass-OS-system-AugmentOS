...
The `LedModule` class provides comprehensive LED control functionality for MentraOS Apps, including RGB LED control, pattern methods, and purpose-specific LED management. It automatically handles request tracking and provides fire-and-forget LED control with immediate resolution.
...
// Control specific LED by purpose
await session.led.turnOn({ 
  color: 'green', 
  ontime: 1000,
...
Create a pulsing effect by rapidly blinking the LED with varying timing.
...
**Parameters:**
- `color`: LED color to use
- `duration`: Total duration of pulse effect (ms)
...
// Long breathing effect
await session.led.pulse('white', 5000);

// Quick status pulse
await session.led.pulse('green', 1000);
```
...
await session.led.userFeedback({ color: 'blue', ontime: 1500 });
```
...
interface LedControlOptions {
  ...
  ledId?: string;
  /** LED purpose to control (e.g., "user_feedback", "general") */
  purpose?: "user_feedback" | "general";
}
```
...
class RecordingLEDApp extends AppServer {
  ...
  private async startRecording(session: AppSession): Promise<void> {
    ...
    // Use privacy LED for recording indication
    await session.led.privacy({ color: 'white', ontime: 30000 });
