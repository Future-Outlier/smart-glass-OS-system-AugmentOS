...
case "process_data": {
  ...
  processDataAsync(userId, toolParameters).then(result => {
    ...
    this.notifyUser(userId, `✅ Processing complete: ${result}`)
  })
...
case "delete_item": {
  ...
  try {
    ...
    if (!item) {
      return "❌ Item not found. Say 'list items' to see what you have."
    }
...
<AccordionGroup>
  <Accordion title="Write clear tool descriptions">
    ...
    ✅ Good: "Create a new item in the user's list with a name and optional category"
  </Accordion>

  <Accordion title="Keep activation phrases natural">
    Use phrases people actually say:

    ❌ Bad: "invoke item creation"

    ✅ Good: "add a", "create", "make a new"
  </Accordion>
  ...
  <Accordion title="Return helpful responses">
    ...
    ✅ Good: "Added 'Buy milk' to your shopping list, due tomorrow"
  </Accordion>
...
<CardGroup cols={3}>
  <Card title="Build a Complete App" icon="rocket" href="/cookbook/voice-timer">
    Try building a full app with these patterns
  </Card>

  <Card title="API Reference" icon="book" href="/reference/interfaces/tool-types">
